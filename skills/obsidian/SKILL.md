...
| Section | Purpose |
| --- | --- |
| `# Summary` | Durable bio, led by a `*Summary refreshed: [[date]].*` marker (the `validate-vault` freshness signal; see people-notes convention). Refreshed by `refresh-person` from `# My notes` + `# From LinkedIn`. |
